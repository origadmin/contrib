...
var _ cache.CacheStorage = (*mockCache)(nil)

func createTestProvider(t *testing.T, c cache.CacheStorage) authnFactory.Provider {
	securityConfig := &securityv1.Security{
		Authn: &authnv1.Authenticator{
			Type: "jwt",
			Config: &authnv1.Authenticator_Jwt{
				Jwt: &jwtv1.Config{
					SigningMethod: "HS256",
	...
	provider, err := authnFactory.Create(securityConfig, WithCache(c))
...
func TestJWTProvider_Success(t *testing.T) {
	...
	require.NoError(t, resp.GetPayload().GetToken().UnmarshalTo(&tokenCred))
